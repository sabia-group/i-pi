...
class QReplicaExchange(Smotion):
    ...
    def __init__(
        ...
        self.stride = int(
            stride
        )  # how often to attempt exchanges (in steps) on average.
    ...
    def step(self, step=None):
        ...
        if mode == "all":
            ...
            pairs = [(temp_to_sys[a], temp_to_sys[b]) for a, b in temp_pairs]
            ...
            if u < p_random:
                ...
                temp_pairs = [
                    (temp_idx[k], temp_idx[k + 1]) for k in range(0, N - 1, 2)
                ]
            ...
            raise ValueError(
                f"Unknown sim_mode '{self.sim_mode}'. Use: nn, nnn, mix, all."
            )
        t_pairs += perf_counter()
        ...
        if (
            fxc
        ):  # write replica indices to PARATEMP file if at least one exchange has been made
