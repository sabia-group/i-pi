...
class Elements(dict):
    ...
    Attributes:
        ...
        atomic_number_list: A dictionary containing the atomic number of different elements.
            Has the form {"label": atomic_number (integer)}.
    """
    ...
    atomic_number_list = {
        "H" : 1,
        "Z" : 1,  # an interpolated H-D atom
        "D" : 1,
        "H2": -1, # Is it meaningfull to define a charge for this "element"?
        "T" : -1, # Is it meaningfull to define a charge for this "element"?
        "He": 2,
        "Li": 3,
        "Be": 4,
        "B" : 5,
    ...
    @classmethod
    def atomic_number(cls, label):
        """Function to access the atomic_number_list attribute.
        Note that this does not require an instance of the Elements class to be
        created, as this is a class method. Therefore using Elements.atomic_number(label)
        will give the atomic number of the element with the atomic symbol given by label.
        Args:
            label: The atomic symbol of the atom whose mass is required.
        Returns:
            An int giving the atomic number of the atom with atomic symbol label.
        """

        try:
            return cls.atomic_number_list[label] # in unit of elementary charges (atomic number)
        except KeyError:
            info("Unknown element given, you must specify the atomic number (in file units.py)", verbosity.low)
            return 0
