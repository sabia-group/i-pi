...
from .qremd import InputQReplicaExchange
...
class InputSmotionBase(Input):
    ...
    attribs = {
        "mode": (
            InputAttribute,
            {
                ...
                "options": ["dummy", "remd", "metad", "dmd", "qremd"],
            },
        )
    }
    fields = {
    ...
    def store(self, sc):
        ...
        if type(sc) is Smotion:
            ...
            self.mode.store("dmd")
            self.dmd.store(sc)
        elif type(sc) is QReplicaExchange:
            self.mode.store("qremd")
            self.qremd.store(sc)
        else:
    ...
    def fetch(self):
        ...
        elif self.mode.fetch() == "qremd":
            sc = QReplicaExchange(**self.qremd.fetch())
        else:
            sc = Smotion()
